...
class RPSubBannerView: UIView, BannerDisplayProtocol {
    var showTime: Float?
    ...
    class func bannerForView(addView: UIView) -> RPSubBannerView? {
        var retView: RPSubBannerView? = nil
        addView.subviews.forEach { (subView) in
            if subView.isKind(of: RPSubBannerView.self as AnyClass) {
                retView = subView as? RPSubBannerView
            }
        }
        return retView
    }
